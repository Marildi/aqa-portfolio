...
def test_country_query_returns_expected_fields():
    query = """
    query {
      country(code: "PT") {
        name
    ...
    response = requests.post(
        "https://countries.trevorblades.com/", json={"query": query}
    )
    assert response.status_code == 200

    data = response.json()["data"]["country"]
    assert data["name"] == "Portugal"
    assert data["capital"] == "Lisbon"


def test_invalid_country_code_returns_error():
    query = """
    query {
      country(code: "ZZ") {
        name
    ...
    data = response.json()
    assert "errors" in data or data["data"]["country"] is None


def test_aws_region():
    query = """
    query getAwsRegion($code: ID!){
      country(code: $code) {
        awsRegion
        }
    }
    ...
    response = requests.post(
        ...
        json={"query": query, "variables": {"code": "EG"}},
    )
    assert response.status_code == 200

    data = response.json()
    assert "errors" not in data
    assert data["data"]["country"]["awsRegion"] is not None


def test_errors_representation():
    query = """
        query getAwsRegion($code: ID!){
          country(code: $code) {
            awsRegion
    ...
    response = requests.post(
        "https://countries.trevorblades.com/",
        json={"query": query, "variables": {"code": "AA"}},
    )
    assert response.status_code == 200

    data = response.json()
    assert "errors" not in data or data["data"]["country"] is None
...
def test_language():
    ...
    data = response.json()
    assert "errors" not in data
    assert data["data"]["country"]["languages"] is not None
    assert len(data["data"]["country"]["languages"]) > 0
